...
class Array(Expr):
    ...
    def evaluate(self,psstacks):
        ...
        arrValueList = ArrayValue(list(reversed(arrList)))
        psstacks.opPush(arrValueList)
...
class Name(Expr):
    ...
    def evaluate(self,psstacks):
        ...
        if self.var_name[0] == '/':
            ...
            var = psstacks.lookup(self.var_name)
            if isinstance(var, FunctionValue):
                count = len(psstacks.dictstack)-1
                name = '/' + self.var_name
                # found = False
                index = 0
                while (count >= 0):
                    if name in psstacks.dictstack[count][1]:
                        index = count
                        # found = True
                    count -= 1
                # if index >= 0:
...
class Block(Expr):
    ...
    def evaluate(self, psstacks):
        ...
        for v in self.value:
            exprList.append(v)
        functValue = FunctionValue(exprList)
        psstacks.opPush(functValue)
...
class FunctionValue(Value):
    ...
    def apply(self, psstacks):
        ...
        # self.body.evaluate(psstacks)

    def __str__(self):
        return '<FunctionValue {}>'.format(self.body)